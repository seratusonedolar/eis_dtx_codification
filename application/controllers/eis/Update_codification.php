...
class Update_codification extends MY_Controller
{
	private $class_link = 'eis/update_codification';
	...
	public function partialtablesearch()
	{
        ...
        // $buyerQuery = "
        //     SELECT buyer 
        //     FROM datatex_hierarchy_buyer 
        //     WHERE user_id = ?
        //     LIMIT 1
        // ";
        // $buyerResult = $this->db->query($buyerQuery, [$this->user_id])->row_array();
        // $buyer = $buyerResult['buyer'];

        $userIdQuery = "
            select user_id from datatex_hierarchy_buyer where buyer in (
			SELECT buyer FROM datatex_hierarchy_buyer WHERE user_id = ? )
        ";

        $userIds = $this->db->query($userIdQuery, [$this->user_id])->result_array();
        $userIdArray = array_column($userIds, 'user_id');

        if (empty($userIdArray)) {
            $userIdArray = [$this->user_id];
        }

        $arrayInteger = array_map('intval', $userIdArray);

		$qString = "select a.dtmitem_id, count(a.dtmitem_id) as countitem from datatex_m_item_detail a
		left join datatex_m_item b on b.dtmitem_id = a.dtmitem_id 
		where 
		b.dtmsubcode_id = ?
		group by a.dtmitem_id;";

		$data['dtmitems'] = $this->db->query($qString, array($id))->result_array();
		$dtmitem_ids = array();
			...
			$result = $this->db->query(
				...
				WITH es AS (
					SELECT  a.dtmitem_id, a.item_id, a.dtmitem_created_by, 
							...
							i.name as item_name,a.dtmitem_description,
							CASE 
	...
	public function updatesubcode(){
		...
		// if($validate){
			...
			dtmitemdtl_updated_at = '$datenows'
			WHERE dtmitem_id = ? 
			--AND dtmitemdtl_code = ? 
			AND dtmsubcodedtl_id = ?";
	...
	public function checkuser($user){
		...
		if($row->user_id == 1033 OR $row->user_id == 404 AND $row->dtmrole_id == 2 OR $row->dtmrole_id == 1){
			return true;
		} else {
			return false;
		}
