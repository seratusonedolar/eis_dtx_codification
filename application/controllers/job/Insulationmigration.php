...
class Insulationmigration extends CI_Controller
{
	public function run($startCounter)
	{
		// $this->processdataByTemplate($startCounter);
		$this->processdataByTemplateWithItemId($startCounter);
	}

	private function templatefabQA()
	{
		return [
			'COMPANYCODE'	=>	'BAG',
	...
	private function collectdatafab()
	{
		// $q = $this->db->query("
		...
		$q = $this->db->query("
			...
			LEFT JOIN datatex_m_subcode dms on dms.dtmsubcode_id = dmi.dtmsubcode_id 
			LEFT JOIN datatex_m_subcode_detail dmsd on dmsd.dtmsubcodedtl_id = dmid.dtmsubcodedtl_id 
			WHERE dmi.dtmsubcode_id = 12 
			AND dmi.dtmitem_code IN (
				select dmi2.dtmitem_code from datatex_m_item dmi2
				where dmi2.dtmsubcode_id = 12
				group by dmi2.dtmitem_code
				having count(dmi2.dtmitem_id) = 1
			)
			union all
	...
	private function processdata()
	{
		...
		foreach ($groupByitems as $dt => $el) {
			...
			$merge1 = array_merge(['FAB'], $lineCode);
	...
	/** Sesuai template QA Server */
	private function processdataByTemplateWithItemId($IMPORTAUTOCOUNTER)
	{
		$datas = $this->collectdatafab();
		$templates = $this->templatefabQA();
		$groupBydtmitemId = $this->groupBydtmitemId($datas);
		...
		foreach ($groupBydtmitemId as $eGroup => $elements) {
			...
			// Values + item detail information 
			$lineMaterial = array_values($templates);
			// $itemEIS = [$groupBydtmitemId[$eGroup][0]['item_id']];
			array_push($lineMaterial, $groupBydtmitemId[$eGroup][0]['item_id']);
			
			$txtLineVal = implode(';', $lineMaterial);
			$this->createLinecsv($txtLineVal);
		}
		return true;
	}
	...
	private function generateHeaderCsv()
	{
		...
		if (!write_file('./public/csv/' . date('ymd') . 'ins.csv', $txtLineVal . PHP_EOL, 'a')) {
