		...
		<tbody>
			...
			foreach ($groupDtmitemfilters as $dtmitem_id => $elements) : ?>
				<tr data-dtmsubcode-id="<?= $groupDtmitemfilters[$dtmitem_id][0]['dtmsubcode_id'] ?>" data-approval="<?= $groupDtmitemfilters[$dtmitem_id][0]['approval_status'] ?>">
					...
					<td><?= $groupDtmitemfilters[$dtmitem_id][0]['item_desc'] ?></td>
					...
					<td style="text-align: center;">
						<select class="form-control form-control-sm" id="approval_status_<?=$groupDtmitemfilters[$dtmitem_id][0]['dtmitem_id']?>" name="approval_status" onchange="appItem(this.value)"
						<?php if ($groupDtmitemfilters[$dtmitem_id][0]['approval_status'] == 'confirmed') {echo "style='color:blue'";}else{echo "style='color:red'";} ?>
						>
							<option value="<?=$groupDtmitemfilters[$dtmitem_id][0]['dtmitem_id']."_confirmed"?>" <?php if ($groupDtmitemfilters[$dtmitem_id][0]['approval_status'] == 'confirmed') {echo "selected";} ?> style="color:blue">Approve</option>
							<option value="<?=$groupDtmitemfilters[$dtmitem_id][0]['dtmitem_id']."_pending"?>" <?php if ($groupDtmitemfilters[$dtmitem_id][0]['approval_status'] == 'pending') {echo "selected";} ?> style="color:red">Pending</option>
						</select>
						<!-- <?php if ($groupDtmitemfilters[$dtmitem_id][0]['approval_status'] != 'pending'): ?>
							<button class="btn btn-danger" onclick="unapproveItem(this)" data-dtmsubcode-id="<?= $groupDtmitemfilters[$dtmitem_id][0]['dtmsubcode_id'] ?>" data-dtmitem-id="<?= $groupDtmitemfilters[$dtmitem_id][0]['dtmitem_id']  ?>">Revoke approval</button>
						<?php else: ?>
...
$(document).ready(function() {
	...
	// 				$.ajax({
	// 						url: '<?php echo base_url() . 'eis/datatech_approval/autocomplete_techinf_hierarchy?dtmsubcodetechinf_id='?>' + dtmsubcodetechinf_id,
	// 						type: 'GET',
	// 						success: function(response) {
	// 							//alert(response)
...
function appItem(val){
	const valid = val.split("_");
	const dtmitem_id = valid[0];
	const valu = valid[1];
	const cselect = $(this);
		$.ajax({
			url: '<?php echo base_url() . $class_link . '/appvalstatus' ?>',
			type: 'POST',
			data: {
				dtmitem_id: dtmitem_id,
				app_val: valu
			...
			success: function(response) {
				...
				if (responseData.status == 200) {
					...
					$("#approval_status_"+dtmitem_id).val(dtmitem_id+'_pending');
					alert(responseData.message);
				}
			},
			beforeSend: function() {
